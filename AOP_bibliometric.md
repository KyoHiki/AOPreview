...
```
    ...
    ##                              Paper           TC TCperYear
    ## 734   ANKLEY GT, 2010, ENVIRON TOXICOL CHEM 981     109.0
    ## 618        VILLENEUVE DL, 2014, TOXICOL SCI 190      38.0
    ## 689              VINKEN M, 2013, TOXICOLOGY 168      28.0
    ## 732     SIPES NS, 2011, BIRTH DEFECTS RES C 144      18.0
    ## 617 TOLLEFSEN KE, 2014, REGUL TOXICOL PHARM 122      24.4
...
Year_plot <-  ggplot (Year) +geom_bar(aes(x=Year,y=Frequency),stat = "identity", position = "dodge")+  theme_classic(base_size = 28)+ theme(axis.text=element_text(colour = "black"),legend.position = c(0.01, 1), legend.justification = c(0, 1))+labs(fill = "") + geom_text( aes(x=Year, y=Frequency,label=Frequency),vjust=-0.3,size=5,col="black")
...
Area_plot <-  ggplot (Area2[1:10,]) + 
  ...
  theme( axis.text=element_text(colour = "black"),axis.text.x=element_text(angle=70,hjust=1 ),legend.position = c(0.01, 1), legend.justification = c(0, 1) ) +
  labs(fill = "") + 
  ylim(0,500)+
  geom_text( aes(x=Category, y=Frequency, label=Frequency),vjust=-0.3,size=4.5,col="black")
...
Journal_plot <- ggplot (Journal2[1:10,]) +geom_bar(aes(x=Journal,y=Frequency),stat = "identity", position = "dodge")+  theme_classic(base_size = 28)+ theme(axis.text=element_text(colour = "black"),axis.text.x=element_text(angle=70,hjust=1 ),legend.position = c(0.01, 1), legend.justification = c(0, 1))+labs(fill = "")+
 geom_text( aes(x=Journal, y=Frequency, label=Frequency),vjust=-0.3,size=4.5,col="black")





ggpubr::ggarrange(Year_plot, Area_plot, nrow=2,ncol=1,labels=LETTERS, font.label=list(size=23))
```
...
net_co_occ <-  networkPlot(NetMatrix_co_occ , normalize="association", weighted=T, n = 45, Title = "Keyword Co-occurrences", type = "fruchterman", size=T,edgesize = 3,labelsize=0.8)
```
...
![](AOPbiblio_figs/AOPbiblio-network%20plot%20for%20all-3.png)<!-- -->
